...
# Safe string replacement for the sidebar
# The old sidebar starts with <div class="tree-group space-y-1 hidden" id="tree-doc1">
# Let's find its end by counting divs if regex is hard, or using BS4 to find it and replace it.

soup = BeautifulSoup(text, 'html.parser')
old_tree = soup.find(id='tree-doc1')

if old_tree:
    new_soup = BeautifulSoup(new_tree_html, 'html.parser')
    old_tree.replace_with(new_soup)
    
    with open(file_path, 'w', encoding='utf-8') as f:
        # Use str(soup) to save it. We'll use HTML5 formatter if needed, but str(soup) works.
        # But wait, str(soup) might mess up some formatting.
        pass

# Better approach: string manipulation using index
start_idx = text.find('<div class="tree-group space-y-1 hidden" id="tree-doc1">')
if start_idx != -1:
    # Find the next comment <!-- Document 2 Tree Directory -->
    end_idx = text.find('<!-- ================= DOCUMENT 2: 实施方案（全量原文） ================= -->', start_idx)
    if end_idx == -1:
      end_idx = text.find('<!-- Document 2 Tree Directory -->', start_idx)
      
    if end_idx != -1:
        # We replace everything from start_idx up to end_idx with new_tree_html
        new_text = text[:start_idx] + new_tree_html + "\n\n        " + text[end_idx:]
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_text)
        print("Success: Sidebar replaced via string index")
    else:
        print("Error: Could not find end_idx for Document 2 Tree Directory")
else:
    print("Error: Could not find tree-doc1 start")
